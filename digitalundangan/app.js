...
const App = (() => {
  ...
  function addToCart(product) {
    ...
    else cart.push({ id: product.id, name: product.name, price: product.price || '-', path: buildPath(product), qty: 1 });
    renderCart();
  ...
  function renderCart() {
    ...
    if (fabCount) fabCount.textContent = String(totalQty);

    if (!cart.length) {
      list.innerHTML = '<p class="cart-empty">Keranjang masih kosong.</p>';
      return;
    }

    list.innerHTML = cart.map((item, i) => `
      <div class="cart-item">
        <div><strong>${item.name}</strong><small>${item.path}</small><small>${item.price}</small></div>
        <div class="cart-item-actions">
          <button onclick="App.decCart(${i})">−</button>
          <span>${item.qty}</span>
          <button onclick="App.incCart(${i})">+</button>
        </div>
      </div>
    `).join('');
  }

  function incCart(i){ if(!cart[i])return; cart[i].qty+=1; renderCart(); }
  function decCart(i){ if(!cart[i])return; cart[i].qty-=1; if(cart[i].qty<=0) cart.splice(i,1); renderCart(); }
  ...
  function checkoutCart() {
    ...
    const lines = cart.map((item, i) => `${i+1}. ${item.path} x${item.qty} (${item.price})`);
    const msg = `Halo, saya mau order produk berikut:%0A${encodeURIComponent(lines.join('\n'))}%0A%0AMohon info lanjut ya 🙏`;
  ...
  // ── PUBLIC ──────────────────────────────────
  return { init, goBack, goToRoot, goToIndex, drillInto, incCart, decCart };
})();

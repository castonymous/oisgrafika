    ...
    if (isset($_POST['save'])) {
        ...
        if (!empty($_FILES['image']['name']) && $_FILES['image']['error'] === 0) {
            ...
            if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'])) {
                ...
                $newName = 'cat_' . time() . '_' . rand(100,999) . '.' . $ext;
        ...
        if ($id > 0) {
            if ($imagePath) {
                $pdo->prepare("UPDATE categories SET name = ?, slug = ?, icon = ?, description = ?, image_path = ? WHERE id = ?")
                    ->execute([$name, $slug, $icon, $desc, $imagePath, $id]);
            } else {
            ...
            $pdo->prepare("INSERT INTO categories (name, slug, icon, description, image_path) VALUES (?, ?, ?, ?, ?)")
                ->execute([$name, $slug, $icon, $desc, $imagePath]);
...
$category = ['id' => 0, 'name' => '', 'icon' => '', 'description' => '', 'image_path' => ''];
if ($action === 'edit' && $id > 0) {
    ...
    <div class="dash-content">
        <?php if ($action === 'add' || $action === 'edit'): ?>
            ...
            <form method="POST" enctype="multipart/form-data" style="max-width: 500px; margin-top: 20px;">
                ...
                <div class="form-group">
                    <label class="form-label">Icon (emoji) - fallback kalau ga ada gambar</label>
                ...
                <div class="form-group">
                    <label class="form-label">Gambar Kategori (opsional, override emoji)</label>
                    <input type="file" name="image" class="form-input" accept="image/*">
                    <div class="form-help">Rekomen ukuran 200×200px, format JPG/PNG/WEBP. Kalau diisi, akan dipake daripada emoji.</div>
                    <?php if (!empty($category['image_path'])): ?>
                        <div style="margin-top: 8px;">
                            <img src="<?= SITE_URL ?>/<?= clean($category['image_path']) ?>" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px; border: 1px solid var(--border);">
                            <span style="font-size: 11px; color: var(--text-muted); margin-left: 8px;">Gambar saat ini</span>
                        </div>
                    <?php endif; ?>
                </div>

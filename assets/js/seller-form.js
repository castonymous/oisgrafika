...
function toggleVariations(enabled) {
    ...
    if (!enabled) {
        // PENTING: Bener-bener bersihkan data variasi biar gak ke-submit lagi
        // (kalau cuma hide CSS, data masih ada di DOM dan akan terkirim)
        ...
        // Trigger update state (tier section auto-show kembali, harga utama auto-enable)
        updatePricingState();
        return;
    }
...
function previewVarImg(input) {
    ...
    const reader = new FileReader();
    reader.onload = (e) => {
        // CRITICAL FIX: JANGAN replace innerHTML (itu akan hapus file input aslinya
        // yang ngebawa file user). Cara benar: hapus elemen visual aja (img/span),
        // lalu tambah <img> preview, TAPI keep <input type="file"> asli.
        
        // Hapus semua child kecuali file input
...
function preparePublish() {
    ...
    // Cek harga: TRUST checkbox enable_variation sebagai source of truth
    // Kalau user uncheck checkbox, dianggap mau pakai harga biasa (no variation)
    const enableVarCheckbox = document.getElementById('enableVariation');
    const hasVariation = enableVarCheckbox && enableVarCheckbox.checked;
...
if (productForm) {
    productForm.addEventListener('submit', function(e) {
        const enableVar = document.getElementById('enableVariation');
        const isVariationOn = enableVar && enableVar.checked;
        
        if (!isVariationOn) {
            // PENTING: Kalau variasi OFF, hapus SEMUA data variasi dari DOM
            // biar gak ke-submit data residual dari produk yang sebelumnya punya variasi
            document.querySelectorAll('.var-axis').forEach(axis => axis.remove());
            const combBody = document.getElementById('combinationBody');
            if (combBody) combBody.innerHTML = '';
            const combTable = document.getElementById('combinationTable');
            if (combTable) combTable.style.display = 'none';
            return; // Skip sync, kita udah hapus semua
        }
        
        // Variasi ON: force sync semua axis sebelum kirim
